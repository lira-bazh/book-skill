...
test("main saves matching Yandex Books URLs when enrichment is enabled", async (t) => {
  ...
  assert.ok(
    logs.includes("Skipped 0 book(s) with existing Yandex Books links")
  );
});
...
test("main limits saved Yandex Books URLs with yandex max results option", async (t) => {
  ...
  assert.equal(exitCode, 0);
  assert.deepEqual(saved[0].yandex_books_urls, [
...
test("main reports Yandex enrichment warnings and keeps writing output JSON", async (t) => {
  ...
  assert.deepEqual(errors, [
    'warning: skipped Yandex Books search for "Book One": Yandex timeout'
  ]);
...
test("main reuses existing Yandex Books URLs and searches only missing books", async (t) => {
  ...
  assert.ok(
    logs.includes("Skipped 1 book(s) with existing Yandex Books links")
  );
...
test("main browser workflow uses one shared browser session", async (t) => {
  ...
  const exitCode = await main(
    ["https://www.livelib.ru/reader/LiraLantan/wish", "--out", outPath],
    {
                ...
                <table>
                  <tr>
                    <td class="t-title-col">
                      <a href="viewtopic.php?t=123">Сто лет одиночества kbps</a>
                    </td>
                  </tr>
                </table>
              `
...
test("main browser workflow skips login confirmations when already authenticated", async (t) => {
  const directory = await mkdtemp(join(tmpdir(), "livelib-wishlist-"));
  t.after(async () => {
    await rm(directory, { recursive: true, force: true });
  });
...
test("main browser workflow keeps JSON read, session work, and JSON write order", async (t) => {
            ...
            return {
              ...
              html: `
                <table>
                  <tr>
                    <td class="t-title-col">
                      <a href="viewtopic.php?t=456">Existing Title Author kbps</a>
                    </td>
                  </tr>
                </table>
              `
  ...
  assert.deepEqual(events, [
    ...
    [
      "details-audio",
      "https://www.litres.ru/audiobook/existing-author/existing-title-123"
    ],
