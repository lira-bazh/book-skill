...
description: Use this skill when the user provides a LiveLib wish-list URL like https://www.livelib.ru/reader/<username>/wish and wants to save the books as JSON with title, authors, LiveLib book URL, and matching book links.
---
...
2. Run `scripts/livelib-wish-to-json.mjs` with `--browser`, a persistent profile directory, a sensible `--max-pages` limit, and optional `--max-results`.
...
7. Save the result to the requested JSON path, or use the script default.
8. Tell the user the output path, how many books were saved, how many pages were processed, and enrichment statistics.
...
additional user fields and already filled `yandex_books_urls` or `litres_urls`.
...
- Do not overwrite existing user fields while enriching links.
